...
<div class="w-full">
    ...
    <a type="button" class="mb-5 float-end bg-blue-500 rounded-md px-2 py-2 text-sm font-semibold text-gray-900" href="{{ route('genre.create') }}">Create new</a>
</div>

<div class="basis-full h-0"></div>

<div class="w-full text-sm">
<table class="table border-collapse border border-gray-400 w-full mt-5 mb-5">
    <thead>
    <tr>
    ...
    @forelse ($genres as $genre)
        <tr>
            <td class="border border-gray-300">{{ $genre->id }}</td>
            <td class="border border-gray-300">{{ $genre->name }}</td>
            <td class="border border-gray-300">{{ $genre->created_at }}</td>
            <td class="border border-gray-300">{{ $genre->updated_at }}</td>
            <td>
                <a type="button" class="px-1 text-sm bg-green-500 rounded-md" href="{{ route('genre.show', $genre->id) }}">Show</a>
                <br>
                <a type="button" class="px-1 text-sm bg-orange-500 rounded-md" href="{{ route('genre.edit', $genre->id) }}">Edit</a>
